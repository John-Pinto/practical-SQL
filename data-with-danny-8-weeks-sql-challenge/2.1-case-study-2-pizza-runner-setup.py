...
def _(mo):
    mo.md(r"""
    # [8 Weeks SQL Challenge - Case Study 2 : Pizza Runner](https://8weeksqlchallenge.com/case-study-2/)
...
def _(conn, mo):
    _df = mo.sql(
        ...
        CREATE TABLE pizza_toppings ("topping_id" INTEGER, "topping_name" TEXT);
...
@app.cell
def _(conn, customer_orders, handling_nulls_datatypes, mo):
    _df = mo.sql(
        f"""
        ...
        WITH
        	handling_nulls_datatypes AS (
        		SELECT
        			...
        			STRING_TO_ARRAY(
        				CASE
        					WHEN exclusions ILIKE 'null' THEN NULL
        					WHEN LENGTH(exclusions) >= 1 THEN exclusions
        				END,
        				', '
        			)::INT[] AS exclusions,
        			STRING_TO_ARRAY(
        				CASE
        					WHEN extras ILIKE 'null' THEN NULL
        					WHEN LENGTH(extras) >= 1 THEN extras
        				END,
        				', '
        			)::INT[] AS extras,
        			order_time
...
@app.cell
def _(conn, handling_nulls_datatypes, mo, runner_orders):
    _df = mo.sql(
        f"""
        ...
        WITH
        	handling_nulls_datatypes AS (
        		SELECT
        			...
        			NULLIF(
        				TRIM(REGEXP_REPLACE(duration, '[^0-9]', '', 'g')),
        				''
        			)::INT AS duration_minutes,
        			NULLIF(
        				CASE
        					WHEN cancellation ILIKE '%null%' THEN NULL
        					ELSE cancellation
        				END,
        				''
        			)::VARCHAR(23) AS cancellation
        		FROM
        			runner_orders
        	)
        SELECT
